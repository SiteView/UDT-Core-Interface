...
int CUdtCore::ProcessSendCtrl(CLIENTCONEXT * cxt)
{
	...
	while (true)
	{
		...
		else if (nReturnCode == 125)
		{
			// 打开数据连接开始传输
		}
		else if (nReturnCode == 150)
		{
			// 打开连接
		}
		else if (nReturnCode == 200)
		{
			// 成功
			struct sockaddr_in addr_c;
			nLen = sizeof(addr_c);
			UDT::getsockname(client, (sockaddr*)&addr_c, &nLen);
			char clienthost[NI_MAXHOST];
			char clientservice[NI_MAXSERV];
			sprintf(clienthost, "%s", inet_ntoa(addr_c.sin_addr));
			sprintf(clientservice, "%d", addr_c.sin_port);
			memset(cmd, 0, CMD_SIZE);
			sprintf(cmd, "%s%s%s", "PORT ", clienthost, ",20,7777");
			if (UDT::ERROR == UDT::send(client, (char*)cmd, CMD_SIZE, 0))
				return 108;
		}
		...
		else if (nReturnCode == 220)
		{
			// 服务就绪 
		}
		else if (nReturnCode == 221)
		{
			// 退出网络 
		}
		else if (nReturnCode == 225)
		{
			// 打开数据连接
		}
		...
		else if (nReturnCode == 227)
		{
			// 进入被动模式
		}
		else if (nReturnCode == 421)
		{
			// 服务关闭
		}
