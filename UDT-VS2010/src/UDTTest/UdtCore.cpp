...
#endif
{
	...
	while (true)
	{
		...
		if (pThis->VEC_IP.empty())
		{
			pThis->VEC_IP.push_back(cxt->strAddr);
	...
	while (true)
	{
		...
		else if (memcmp(Head,"MSR", 3) == 0)
		{
			...
			if (szFileType == "")
			{
				...
				pThis->m_pCallBack->onAccept((char*)cxt->strAddr, szFileName.c_str(), nCount, 
				szRecdevice.c_str(), szRectype.c_str(), szOwndevice.c_str(), szOwntype.c_str(), 
				szSendType.c_str(), szFileType.c_str(), fhandle);
		}
