...
#ifdef WIN32
int UdtFile::ListDir(const char * path, int64_t & nTotalSize, std::vector<_FileInfo>& vecEntries, std::vector<std::string>& vecDirs)
{
	// check the arguments
...
int UdtFile::ListDir(const char * path, int64_t & nTotalSize, std::vector<_FileInfo>& vecEntries, std::vector<std::string>& vecDirs)
{
	...
	for (;;)
	{
		...
		char strAPath[256];
		char strUPath[256];
		sprintf(strAPath, "%s\\%s", path, entry_pointer->d_name);
		sprintf(strUPath, "%s\\%s", path, entry_pointer->d_name);
		struct stat stat_buffer;
		int nRet = stat(strAPath, &stat_buffer);
		if (nRet == 0)
		{
			_FileInfo info;
			info.asciPath = strAPath;
